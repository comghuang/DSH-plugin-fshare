  ...
  factory: function (require) {
    ...
    function OpsDock(props) {
      ...
      var refresh = function () {
        fetch('/fshare/api?op=status').then(function (r) { return r.json() }).then(function (res) {
          if (res && res.error) { setErr(String(res.error)); setData(null) } else { setErr(''); setData(res) }
        }).catch(function (e) { setErr('连接失败'); setData(null) })
      }
      React.useEffect(function () {
        var id = setInterval(refresh, openValue ? 10000 : 60000)
        return function () { clearInterval(id) }
      }, [openValue])
      var toggle = function () {
        var next = !openValue
        setOpen(next)
        if (next && dataValue === null) refresh()
      }

      var chips = []
      if (dataValue) {
        if (dataValue.cpu >= 0) chips.push('CPU ' + dataValue.cpu + '%')
        if (dataValue.memPct >= 0) chips.push('内存 ' + dataValue.memPct + '%')
        for (var i = 0; i < (dataValue.disks || []).length; i++) {
          var d = dataValue.disks[i]
          var pct = d.total > 0 ? Math.round(100 * (d.total - d.free) / d.total) : -1
          if (pct >= 0) chips.push(d.name + ' ' + pct + '%')
        }
        if (dataValue.gpu && dataValue.gpu.temp) chips.push('GPU ' + dataValue.gpu.util + '% ' + dataValue.gpu.temp + '°C')
      }
      var summary = chips.length ? chips.join(' · ') : '点击查看机器状态'

      var body = null
      if (openValue) {
        var tile = function (label, value, sub) {
          return React.createElement('div', { className: 'fshare-tile' },
            React.createElement('div', { className: 'fshare-tile-label' }, label),
            React.createElement('div', { className: 'fshare-tile-value' }, value),
            sub ? React.createElement('div', { className: 'fshare-tile-sub' }, sub) : null)
        }
        var tiles = []
        if (dataValue) {
          tiles.push(tile('CPU', dataValue.cpu >= 0 ? dataValue.cpu + ' %' : '—', '处理器平均负载'))
          tiles.push(tile('内存', dataValue.memPct >= 0 ? dataValue.memPct + ' %' : '—',
            dataValue.memTotalMB ? dataValue.memUsedMB + ' / ' + dataValue.memTotalMB + ' MB' : ''))
          for (var j = 0; j < (dataValue.disks || []).length; j++) {
            var dj = dataValue.disks[j]
            var pctj = dj.total > 0 ? Math.round(100 * (dj.total - dj.free) / dj.total) : -1
            tiles.push(tile('磁盘 ' + dj.name, pctj >= 0 ? pctj + ' %' : '—',
              dj.total > 0 ? (dj.total - dj.free) + ' / ' + dj.total + ' GB 已用' : ''))
          }
          if (dataValue.gpu && dataValue.gpu.temp) {
            tiles.push(tile('GPU', dataValue.gpu.util + ' %',
              dataValue.gpu.temp + '°C · 显存 ' + dataValue.gpu.memUsed + ' / ' + dataValue.gpu.memTotal + ' MB'))
          }
          if (tiles.length === 0) tiles.push(React.createElement('div', { className: 'fshare-empty' }, '暂无状态数据'))
        }
        body = React.createElement('div', { className: 'fshare-body' },
          React.createElement('div', { className: 'fshare-bar' },
            React.createElement('button', { key: 'r', className: 'fshare-btn', onClick: refresh }, '刷新'),
            React.createElement('span', { key: 'n', className: 'fshare-count' }, '展开时每 10 秒自动刷新')),
          err ? React.createElement('div', { className: 'fshare-err' }, err) : React.createElement('div', { className: 'fshare-tiles' }, tiles))
      }

      return React.createElement('div', { className: 'fshare-dock' },
        React.createElement('button', {
          className: 'fshare-head', onClick: toggle,
          title: openValue ? '收起运维面板' : '展开运维面板',
        },
          React.createElement('span', { className: 'fshare-lead' }, '🖥'),
          React.createElement('span', { className: 'fshare-title' }, '运维'),
          React.createElement('span', { className: 'fshare-summary' }, summary),
          React.createElement('span', { className: 'fshare-chev' + (openValue ? ' fshare-chev-open' : '') }, '▾')),
        body)
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () {
        return slots.inject('conversation.input.dock', function () {
          var d1 = slots.register({
            name: 'conversation.input.dock', id: 'ops', order: 11, label: '运维',
          }, function (props) { return React.createElement(OpsDock, props) })
          var d2 = slots.register({
